...
<head>
    ...
    <style>
        ...
        .large-icon {
            ...
            color: blueviolet; /* Warna default ikon */
        }

        .large-icon:hover {
            color: #000; /* Warna hitam saat kursor diarahkan */
        }
...
<body>
    <div class="container" style="background-color: #7fad39">
        <div class="recipe-header" style="text-align: center; margin-bottom: 30px;">
            ...
            <a onclick="goBack()"><i class="fa-regular fa-circle-left large-icon"></i></a>
            </div>
        </div>

        <h1 class="recipe-title">{{ $resep->nama_resep }}</h1>
        <div style="display: flex; border-radius: 15px; overflow: hidden; ">
            <div id="imageContainer"
                style="flex: 1; display: flex; justify-content: center; align-items: center; position: relative;">
                <img src="{{ asset($resep->gambar) }}" alt="Nama Resep"
                    style="width: 110%; height: 400px; object-fit: cover; border-radius: 20px; margin-left: 10%; margin-right: auto; margin-bottom: 20px; ">
                <div id="recipeActions" class="recipe-actions"
                    style="display: none; position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); background-color: rgba(0, 0, 0, 0.5); padding: 3%; border-radius: 10px; text-align: center; margin-bottom: 20px ; margin-left: 5%; margin-right: auto;">
                    @if(auth()->check())
                    @php
                    ...
                    <form id="favoritForm" action="{{ route('tambah_favorit', $resep->id_resep) }}" method="POST">
                        @csrf
                        @if($favorit)
                        <button type="button" class="favorit-button" style="background-color: #dc3545; color: white;"
                            disabled>
                    ...
                    @else
                    <p style="color: white;">Silakan <a href="{{ route('login') }}" style="color: #007bff;">masuk</a>
                        atau <a href="{{ route('register') }}" style="color: #007bff;">daftar</a> untuk menambahkan
                        resep ke favorit.</p>
                    @endif
                </div>
            </div>
            <div style="flex: 1; padding: 20px; display: flex; flex-direction: column; align-items: flex-start;">
                <h2
                    style="color: #ffffff; font-size: 20px; margin-bottom: 20px; margin-left: 35%; margin-right: 25%; background-color: #384246; border-radius: 5px; padding: 10px; text-align: center;">
                ...
                <div class="ingredient-card" style="margin: 0 auto; width: 80%; margin-left: 10%; margin-right: 20%;">
                    <h2 style="color: #333; font-size: 18px; margin-bottom: 10px; text-align: center;">Bahan - Bahan
                    </h2>
                    <ul style="padding-left: 0; list-style-type: none;">
                        @foreach ($bahan as $b)
                        <li style="margin-bottom: 5px;">{{ $loop->iteration }}. {{ $b->nama_bahan }} {{ $b->takaran }}
                            {{ $b->satuan }}</li>
                        @endforeach
                    </ul>
                </div>
        ...
        <div class="nutrition-card"
            style="font-family: Arial, sans-serif; margin-bottom: 20px; padding: 2%; width: 320px; margin-top: 20px; background-color: #f9f9f9; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
            <h2 style="font-size: 24px; margin-bottom: 20px; color: #333; text-align: center;">Informasi Nilai Gizi :
            </h2>
            <div style="display: flex; justify-content: space-between;">
                <div
                    style="text-align: center; flex: 0.5; background-color: #ffffff; padding: 5%; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                ...
                <div
                    style="text-align: center; flex: 0.5; background-color: #ffffff; padding: 5%; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); margin-left: 3%;">
        ...
        <div class="ingredient-card"
            ...
            <div style="background-color: #fff; border-radius: 10px; padding: 15px;">
                <p style="font-size: 16px; line-height: 1.6; color: #555; white-space: pre-wrap;">{{
                    $resep->cara_pembuatan }}</p>
            </div>
        </div>
